#!/usr/bin/env python3
"""
Proper, Rigorous FraudBoost vs XGBoost Benchmark - Working Version
=================================================================

Built from the working mini benchmark with all methodological fixes.
"""
...
print("=== FRAUDBOOST VS XGBOOST PROPER BENCHMARK ===")
...
print(f"✓ Train loaded: {len(train_df):,} rows, fraud rate: {train_df['is_fraud'].mean()*100:.3f}%")

print(f"Loading test dataset from {test_path}...")
test_df = pd.read_csv(test_path)
print(f"✓ Test loaded: {len(test_df):,} rows, fraud rate: {test_df['is_fraud'].mean()*100:.3f}%")
...
print(f"Target: {target_size:,} rows with {fraud_rate*100:.3f}% fraud rate")
...
# Feature engineering
print("\n=== FEATURE ENGINEERING ===")
print("Engineering features for train subsample...")
...
print(f"Val: {len(X_val):,} rows, fraud rate: {y_val.mean()*100:.3f}%")
...
print(f"Calculated positive weight ratio: {pos_weight:.2f}")
...
# Results reporting
print("\n" + "="*60)
print("=== HELD-OUT TEST RESULTS (Default Threshold 0.5) ===")
print("="*60)
...
print("\n" + "="*60)
print("=== HELD-OUT TEST RESULTS (Optimal Threshold) ===")
print("="*60)
...
print(opt_results_df[cols].to_string(index=False))
...
print("Calculating precision at fixed recall levels...")
...
print(f"Precision@80%Recall: FraudBoost {fb_p80*100:.2f}%, XGBoost {xgb_p80*100:.2f}%")
print(f"Precision@90%Recall: FraudBoost {fb_p90*100:.2f}%, XGBoost {xgb_p90*100:.2f}%")
print(f"(Actual recall achieved: FB 80%={fb_r80*100:.1f}%, 90%={fb_r90*100:.1f}%; XGB 80%={xgb_r80*100:.1f}%, 90%={xgb_r90*100:.1f}%)")
...
print(xgb_cal_df[['bin', 'count', 'avg_confidence', 'accuracy', 'gap']].round(4).to_string(index=False))

print(f"\nFraudBoost Calibration:")
fb_cal_df = pd.DataFrame(fb_cal_table)
print(fb_cal_df[['bin', 'count', 'avg_confidence', 'accuracy', 'gap']].round(4).to_string(index=False))

print(f"\nECE: FraudBoost {fb_ece:.4f}, XGBoost {xgb_ece:.4f}")

# FP cost sensitivity analysis
print("\n" + "="*60)
print("=== FP COST SENSITIVITY ===")
print("="*60)

fp_costs = [10, 25, 50, 75, 100, 150, 200, 500, 1000]
sensitivity_results = []

print("Running FP cost sensitivity analysis...")
for i, fp_cost in enumerate(fp_costs):
    print(f"  Testing FP cost ${fp_cost} ({i+1}/{len(fp_costs)})...")
...
print(f"FraudBoost AUC: {np.mean(fb_cv_aucs):.4f} ± {np.std(fb_cv_aucs):.4f}")
print(f"XGBoost AUC: {np.mean(xgb_cv_aucs):.4f} ± {np.std(xgb_cv_aucs):.4f}")
print(f"FraudBoost Net Savings: ${np.mean(fb_cv_savings):,.0f} ± ${np.std(fb_cv_savings):,.0f}")
print(f"XGBoost Net Savings: ${np.mean(xgb_cv_savings):,.0f} ± ${np.std(xgb_cv_savings):,.0f}")
...
print("Results ready for saving to benchmarks/results_proper.md")